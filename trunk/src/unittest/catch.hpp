...
namespace Catch {
    ...
    {
    public:
        explicit NotImplementedException( SourceLineInfo const& lineInfo );
...
public:
  explicit MethodTestCase( void (C::*method)() ) : m_method( method ) {}
...
struct NameAndDesc {
  explicit NameAndDesc( const char* _name = "", const char* _description= "" )
...
public:
  ...
  explicit  ExpressionResultBuilder( ResultWas::OfType resultType = ResultWas::Unknown );
...
namespace Catch {
    ...
    public:
      ...
      explicit TagExtracter( std::set<std::string>& tags )
        :   m_tags( tags )
        {}
    ...
    public:
      explicit TagExpressionParser( TagExpression& exp )
        :   m_isNegated( false ),
            m_exp( exp )
        {}
...
namespace Catch {
    ...
    public:
      explicit TestCaseFilter( std::string const& testSpec, IfFilterMatches::DoWhat matchBehaviour = IfFilterMatches::AutoDetectBehaviour )
...
#pragma clang diagnostic pop
    ...
    class TestCaseFilters {
    public:
      explicit TestCaseFilters( std::string const& name ) : m_name( name ) {}
...
namespace Catch {
    ...
    public:
      ...
      explicit Config( ConfigData const& data )
        :   m_data( data ),
            m_os( std::cout.rdbuf() )
        {
...
namespace Catch {
    ...
    public:
      ...
      explicit Option( T const& _value )
...
namespace Catch
{
    ...
    struct TestRunInfo {
      explicit TestRunInfo( std::string const& _name ) : name( _name ) {}
        std::string name;
    };
    struct GroupInfo {
    ...
    struct TestGroupStats {
        ...
        :   groupInfo( _groupInfo ),
            ...
            aborting( _aborting )
        {}
      explicit TestGroupStats( GroupInfo const& _groupInfo )
        :   groupInfo( _groupInfo ),
            aborting( false )
        {}
    ...
    struct StreamingReporterBase : SharedImpl<IStreamingReporter> {
      ...
      explicit StreamingReporterBase( ReporterConfig const& _config )
    ...
    struct CumulativeReporterBase : SharedImpl<IStreamingReporter> {
      ...
      explicit CumulativeReporterBase( ReporterConfig const& _config )
        :   m_config( _config.fullConfig() ),
            stream( _config.stream() )
        {}
...
public:
  ...
  CompositeGenerator( const CompositeGenerator& other )
    :   m_fileInfo( const_cast <CompositeGenerator&> (other).m_fileInfo ),
        m_totalSize( 0 )
    {
        move( other );
    }
...
namespace Catch {
    ...
    class ExceptionTranslatorRegistrar {
	  ...
	  explicit ExceptionTranslator( std::string(*translateFunction)( T& ) )
...
namespace Matchers {
    ...
    namespace StdString {
	  ...
	  explicit Equals( std::string const& str ) : m_str( str ){}
	  ...
	  explicit Contains( std::string const& substr ) : m_substr( substr ){}
	  ...
	  explicit StartsWith( std::string const& substr ) : m_substr( substr ){}
	  ...
	  explicit EndsWith( std::string const& substr ) : m_substr( substr ){}
...
namespace Catch {
    ...
    class Text {
    public:
      explicit Text( std::string const& _str, TextAttributes const& _attr = TextAttributes() );
...
namespace Clara {
    namespace Detail {
        ...
        public:
	  explicit BoundArgFunction( IArgFunction<ConfigT>* _functionObj ) : functionObj( _functionObj ) {}
	  ...
	  explicit BoundDataMember( M C::* _member ) : member( _member ) {}
	  ...
	  explicit BoundUnaryMethod( void (C::*_member)( M ) ) : member( _member ) {}
            virtual void set( C& p, std::string const& stringValue ) const {
	  ...
	  explicit BoundNullaryMethod( void (C::*_member)() ) : member( _member ) {}
            virtual void set( C& p, std::string const& stringValue ) const {
                bool value;
                convertInto( stringValue, value );
                if( value )
	  ...
	  explicit BoundUnaryFunction( void (*_function)( C& ) ) : function( _function ) {}
	  ...
	  explicit BoundBinaryFunction( void (*_function)( C&, T ) ) : function( _function ) {}
            virtual void set( C& obj, std::string const& stringValue ) const {
    ...
    class CommandLine {
        ...
        struct Arg {
	  explicit Arg( Detail::BoundArgFunction<ConfigT> const& _boundField ) : boundField( _boundField ), position( -1 ) {}
        ...
        public:
            ...
            {}
	  ArgBinder( const ArgBinder& other )
            :   m_cl( const_cast <ArgBinder&> (other).m_cl ),
                m_arg( const_cast <ArgBinder&> (other).m_arg )
            {
	      const_cast <ArgBinder&> (other).m_cl = NULL;
            }
            ~ArgBinder() {
                if( m_cl ) {
                    m_arg.validate();
...
namespace Catch {
    ...
    struct Colour {
      ...
      explicit Colour( Code _colourCode );
        ~Colour();
...
namespace SectionTracking {
    ...
    public:
      explicit TestCaseTracker( std::string const& testCaseName )
        ...
        class Guard {
        public:
	  explicit Guard( TestCaseTracker& tracker )
            : m_tracker( tracker )
            {
...
namespace Catch {
    ...
    public:
      ...
      RunContext( Ptr<IConfig const> const& config, Ptr<IStreamingReporter> const& reporter )
        ...
        virtual ~RunContext() {
            ...
            m_context.setConfig( Ptr <const IConfig> (NULL) );
...
namespace Catch {
    ...
    public:
      explicit Runner( Ptr<Config> const& config )
        :   m_config( config )
        {
            openStream();
            makeReporter();
        ...
        Totals runTests() {
            ...
            RunContext context(Ptr <const IConfig> (m_config.get()), m_reporter );
    ...
    private:
        ...
        void makeReporter() {
            ...
            m_reporter = getRegistryHub().getReporterRegistry().create( reporterName, Ptr <IConfig> (m_config.get() ));
            if( !m_reporter ) {
                std::ostringstream oss;
...
namespace Catch {
    ...
    public:
      ...
      explicit FreeFunctionTestCase( TestFunction fun ) : m_fun( fun ) {}
...
namespace Catch {
    ...
    struct GeneratorInfo : IGeneratorInfo {
      ...
      explicit GeneratorInfo( std::size_t size )
        :   m_size( size ),
            m_currentIndex( 0 )
        {}
...
namespace Catch
{
    ...
    {
    public:
      explicit LegacyReporterAdapter( Ptr<IReporter> const& legacyReporter );
...
namespace Catch {
    ...
    class LegacyReporterRegistrar {
        ...
        class ReporterFactory : public IReporterFactory {
	      ...
	      return new LegacyReporterAdapter( Ptr <IReporter> (new T( config )) );
            }
    ...
    public:
      ...
      explicit LegacyReporterRegistrar( std::string const& name ) {
...
namespace Catch {
    ...
    public:
        ...
        class ScopedElement {
        public:
            explicit ScopedElement( XmlWriter* writer )
	      :   m_writer( writer )
            {}
...
namespace Catch {
    ...
    public:
      explicit XmlReporter( ReporterConfig const& config ) : m_config( config ), m_sectionDepth( 0 ) {}
...
namespace Catch {
    ...
    public:
      explicit JunitReporter( ReporterConfig const& _config )
...
namespace Catch {
    ...
    struct ConsoleReporter : StreamingReporterBase {
      explicit ConsoleReporter( ReporterConfig const& _config )
